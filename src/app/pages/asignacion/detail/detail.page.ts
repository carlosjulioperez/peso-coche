...
export class DetailPage implements OnInit {
  ...
  private controles(){
    return new FormGroup({
      id: new FormControl(), 
      fecha_as: new FormControl(), 
      codigo_video_jet: new FormControl(),
  ...
  grabar(){
    ...
    this.setValue('fecha_as', this.utilsService.getDateToString(new Date()));
    this.setValue('estado', EstadoCoche.Tara); //siguiente flujo

    this.registroService.updateAsignacion(this.myForm.value).then(resp=>{
      console.log("Actualizando registro...");
      console.log(resp);
      // alert("Datos actualizados." + resp);
      this.navCtrl.navigateForward('asignacion', {} );
    });
  }
  ...
  getFechaPesaje(): string {
    return this.utilsService.getFechaUTC5(this.registro.fecha_pe);
  }
}
